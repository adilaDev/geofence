...
<!-- <div class="offcanvas offcanvas-start w-50" tabindex="-1" id="offcanvasScrolling" data-bs-scroll="true" aria-labelledby="offcanvasScrollingLabel" style="visibility: hidden;" aria-hidden="true"> -->
<div class="offcanvas offcanvas-start layer-map w-50" data-bs-scroll="true" tabindex="-1" id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel" aria-modal="true" role="dialog" aria-hidden="true">
        ...
        <!-- <form class="p-0"> -->
            <div class="form-group m-0">
                <div class="input-group">
                    <input type="search" oninput="initPlacesSearch()" class="form-control" id="searchInput" style="height: 30px;" placeholder="<?= !empty($this->lang->line('search')) ? $this->lang->line('search') : 'Search'; ?>..."
...
<script>
    ...
    function initTbLayer(){
        ...
        if ($.fn.DataTable.isDataTable('#prevtable-layer-tbl')) {
            // Jika sudah ada, hapus tabel DataTable yang sudah ada
            $('#table-layer').DataTable().destroy();
            $('#table-layer').empty();
            tbody.empty();
        }
        tb_layer = $("#table-layer").DataTable({
            ...
            columns: [
                // { data: '', title: '' },
                // { data: 'id', className: 'text-start', title: '<?= !empty($this->lang->line('id')) ? $this->lang->line('id') : 'id'; ?>' },
                { data: 'no', className: 'text-start', title: '<?= !empty($this->lang->line('no')) ? $this->lang->line('no') : 'No'; ?>' },
                { data: 'polygon', className: 'text-start', title: '<?= !empty($this->lang->line('polygon')) ? $this->lang->line('polygon') : 'Polygon'; ?>' },
                { data: 'polygon_id', className: 'text-start', title: '<?= !empty($this->lang->line('polygon_id')) ? $this->lang->line('polygon_id') : 'Polygon Id'; ?>' },
                { data: 'coordinate', className: 'text-start', title: '<?= !empty($this->lang->line('coordinate')) ? $this->lang->line('coordinate') : 'Coordinate'; ?>'},
            ],
            // columnDefs: [
            //     {
            //         orderable: false,
    ...
    function createTbLayer(listFeatures){
        ...
        if (listFeatures.length > 0) {
            ...
            for (let i = 0; i < listFeatures.length; i++) {
                ...
                const geo = obj.geometry.coordinates[0];
                const prop = obj.properties;
                const id = obj.id;
                // const lat = prop.latitude;
                // const lon = prop.longitude;
                const lat = (prop.latitude == undefined) ? geo[0] : prop.latitude; 
                const lon = (prop.longitude == undefined) ? geo[1] : prop.longitude;
    ...
    function generateTbody(no, id, lat, lon){
        ...
        divPolygon.id = "polygon-"+id;
        divPolygon.setAttribute('onclick', 'zoomPolygon("'+id+'")');
        divPolygon.setAttribute('style', 'cursor: pointer;');
        canvas.appendChild(divPolygon);
        renderPolygonImageById(id, divPolygon);
        
        // var tr = `<tr>
        //     <td>${no}</td>
        //     <td>${canvas.innerHTML}</td>
    ...
    function renderPolygonImageById(polygonId, containerId) {
        ...
        var polygonCoordinates = filterPolygon.geometry.coordinates; // koordinat poligon yang sesuai dari Mapbox
        ...
        var type = (filterPolygon.properties.isCircle) ? 'circle' : filterPolygon.geometry.type;
    ...
    function superDynamicDT(data = []){
        ...
        tables = $(table).DataTable({
            ...
            columns: [
                ...
                {
                    data: 'action', title: "<?= !empty($this->lang->line('action')) ? $this->lang->line('action') : 'Action'; ?>",
                    ...
                    //     //         '<a class="btn btn-outline-danger btn-sm delete" title="<?= !empty($this->lang->line('delete')) ? $this->lang->line('delete') : 'Delete'; ?>"><i class="fas fa-trash-alt"></i></a>';
                    //     return '<a class="btn btn-outline-danger btn-sm delete" title="<?= !empty($this->lang->line('delete')) ? $this->lang->line('delete') : 'Delete'; ?>"><i class="fas fa-trash-alt"></i></a>';
                    // }
                }
            ],
            columnDefs: [{
        ...
        //     var newData = {'no': '', 'key': '', 'value': '', 'action': '<button type="button" class="btn btn-outline-danger btn-sm delete" title="<?= !empty($this->lang->line('delete')) ? $this->lang->line('delete') : 'Delete'; ?>"><i class="fas fa-trash-alt"></i></button>'};
...
<script type="text/javascript">
    ...
    map.addControl(new mapboxgl.AttributionControl({
        customAttribution: `<?= !empty($this->lang->line('footer_mapbox')) ? $this->lang->line('footer_mapbox') : '© Geofence Drawing Web. Design & Developed by <a href="https://asiaresearchinstitute.com/" target="_blank" rel="noopener noreferrer"><img src="https://geofence.asiaresearchinstitute.com/assets/images/LOGO_ARI/logo_ari_green.svg" alt="" width="auto" height="13"></a>'; ?>`
    ...
    map.on("draw.selectionchange", function (e){
        ...
        if (getPolygon.features.length > 0) {
            var getPolygonFeatures = getPolygon.features[0];
            var getIdSelected = getPolygonFeatures.id;
            var tbdata = tb_layer.data().toArray();

            // Filter data
            var filteredData = filterData(tb_layer.data().toArray(), { polygon_id: getIdSelected });

            if (filteredData.length == 0) {
                var center = getCenterPoint(getPolygonFeatures);
                var latlon = parseFloat(center[1]).toFixed(6) + ", " + parseFloat(center[0]).toFixed(6);

                var canvas = document.createElement('canvas');
                var divPolygon = document.createElement('div');
                divPolygon.id = "polygon-" + getIdSelected;
                divPolygon.setAttribute('onclick', 'zoomPolygon("' + getIdSelected + '")');
                divPolygon.setAttribute('style', 'cursor: pointer;');
                canvas.appendChild(divPolygon);
                renderPolygonImageById(getIdSelected, divPolygon);

                var indexes = tb_layer.rows().indexes().toArray();
                // Atur ulang indeks
                indexes.forEach((index, newIndex) => {
                    tb_layer.row(index).index(newIndex);
                    console.log("index: ", index, " new: ", newIndex);
                });
                // Gambar kembali tabel
                // tb_layer.draw();
                console.log("indexOld: ", indexes, " new: ", tb_layer.rows().indexes().toArray());

                // Tambahkan baris baru
                tb_layer.row.add({
                    // id: tb_layer.rows().count() - 1,
                    no: tb_layer.rows().count() + 1, // Nomor baru dihitung berdasarkan jumlah baris saat ini
                    polygon: canvas.innerHTML,
                    polygon_id: getIdSelected,
                    coordinate: latlon,
                }).draw(false);
                resetRowNumbers();
            }

            console.log("getPolygon: ", getPolygonFeatures);
            console.log("center: ", center);
            console.log("filteredData: ", filteredData);
            console.log("TB_DATA: ", tbdata);
            console.log("ROWDATA : ", tb_layer.rows().indexes().toArray());
        }
        ...
        //     // Gambar kembali tabel
        //     tb_layer.draw();
        //     var id_rows_data = tb_layer.rows().indexes().toArray();
        //     var getLastIndex = isNaN(id_rows_data[id_rows_data.length-1]) ? -1 : id_rows_data[id_rows_data.length-1];
        //     var getNextIndex = getLastIndex+=1;
        //     const filteredData = filterData(tbdata, { polygon_id: getIdSelected });
        ...
        //     // saveDataTableToProperties
        //     // createTbLayer
        //     // tambahkan jika belum ada di dalam daftar polygon layer
        //     if (filteredData.length == 0) {
        //         var center = getCenterPoint(getPolygonFeatures);
        //         var latlon = parseFloat(center[1]).toFixed(6)+", "+parseFloat(center[0]).toFixed(6);
        ...
        //         .add({
        //             // id: tb_layer.rows().count() - 1,
        //             // id: id_rows_data.length - 1,
        //             no: id_rows_data.length+1,
        //             polygon: canvas.innerHTML,
        //             polygon_id: getIdSelected,
        //             // coordinate: getPolygonFeatures.geometry.coordinates,
        //             coordinate: latlon,
        //         }).draw();
        ...
        //     console.log("TB_DATA: ", tbdata);
        //     console.log("filteredData: ", filteredData);
        //     console.log("ID_ROW: ", getLastIndex, ' -> ', getNextIndex, " ROWDATA ",id_rows_data, "\nindexes: ", indexes);
        //     // // tb_layer.order([[1, 'asc']]).draw();
        // }
    ...
    map.on("draw.delete", function (e){
        ...
        var id_feat = feat.id;
        var rdl = removeDatatableLayer(id_feat);
        console.log("drawDelete: ", e, "\nafterDelete: ", rdl);
    });
    
    function filterData(array, criteria) {
        // using data Array of Object
        ...
        // Contoh penggunaan:
        // const data = [{ id: 1, name: 'John', age: 30 }, { id: 2, name: 'Jane', age: 25 }, { id: 3, name: 'Doe', age: 35 }];
    ...
    $("#btn_save_polygon").on("click", function (e){
        // e.preventDefault();
        var inputValue = "<?= urldecode($this->uri->segment(3)) ?>";
        console.log("inputValue: ", inputValue);
        console.log("underscore: ", underscore(inputValue));
        console.log("is Japan: ", isJapaneseText(inputValue));
    ...
    function getAPI(lngLat){
        geoAPIgoogle(lngLat).then((data) => {
            ...
            //     geocodeToDataTable.push({'no': no, 'key': "formatted_address_"+no, 'value': value, 'action': '<button type="button" class="btn btn-outline-danger btn-sm delete" title="<?= !empty($this->lang->line('delete')) ? $this->lang->line('delete') : 'Delete'; ?>"><i class="fas fa-trash-alt"></i></button>'});
            // }
    ...
    function removeFeature(){
        ...
        removeDatatableLayer(getId);
        $(".btn-close").click();
    }

    function resetRowNumbers() {
        // Atur ulang nomer urut
        tb_layer.column(0).nodes().each(function(cell, i) {
            cell.innerHTML = i + 1;
        });
    ...
    function removeDatatableLayer(getIdFeature) {
        const filteredData = filterData(tb_layer.data().toArray(), { polygon_id: getIdFeature });
        const tbdata = tb_layer.data().toArray();
        const indexes = tb_layer.rows().indexes().toArray();
        const log = {
        ...
        if (filteredData.length > 0) {
            // const rowId = parseInt(filteredData[0].no) - 1;
            // tb_layer.row(rowId).remove().draw(false);
            
            const rowId = tb_layer.row((idx, data, node) => data.polygon_id === getIdFeature).index(); // Dapatkan indeks baris berdasarkan polygon_id
            log.rowId = rowId;
            if (rowId !== undefined) {
                tb_layer.row(rowId).remove().draw(false);
                resetRowNumbers(); // Atur ulang nomor pada kolom nomer setelah menghapus
                ...
                console.log("Baris dengan polygon_id", getIdFeature, "tidak ditemukan.");
            }
        }
    ...
    //     var tbdata = tb_layer.data().toArray();
    //     var id_rows_data = tb_layer.rows().indexes().toArray();
    //     var getLastIndex = id_rows_data[id_rows_data.length-1];
    //     var getNextIndex = id_rows_data.length;
    //     const filteredData = filterData(tbdata, { polygon_id: getIdFeature });
    //     var getRowId = (filteredData.length == 0) ? 'NULL' : (parseInt(filteredData[0].no)-1);
    //     // var getRowId = (filteredData.length == 0) ? 'NULL' : (filteredData.length-1);
    //     var getRowSel = (getRowId == 'NULL') ? null : tb_layer.row(getRowId).data();
    //     var getRowDel = (getRowId == 'NULL') ? null : tb_layer.row(getRowId).remove().draw(false);

    //     var log = {
    //         "data": tbdata,
    //         "id_rows_data": id_rows_data,
    ...
    function saveInfo(dataInfo = {}){
        $.ajax({
            type: "POST",
            url: "<?=base_url('map/saveInfo')?>",
            data: dataInfo,
            // dataType: "dataType",
            success: function (response, status) {
                console.log("saveInfo: ", status, response);
            },
            error: function (xhr, status, error){
                console.log("saveInfo: ", status, error, xhr);
            }
        });
    }
