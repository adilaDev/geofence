...
<div class="button-layer">
    <div class="mapboxgl-ctrl mapboxgl-ctrl-group" id="previewFeature">
...
<script type="text/javascript">
    ...
    function geoAPIgoogle(centerCoord){
        ...
        var lang = '<?= ($lang == 'jp') ? 'ja' : 'en' ?>'; // en atau ja
    ...
    function superDynamicDT(data = []){
        ...
        Object.keys(data[0]).forEach(function(key) {
            var key_name = key.replace('_', ' ');
            $('<th>').text(key_name).appendTo(headerRow);
        });
        ...
        data.forEach(function (item) {
            ...
            Object.entries(item).forEach(function ([key, value], index) {
                ...
                console.log("valDT: ", key, value, " isEditable: ", isEditable, isNonEditableRow);
            });
        ...
        tables = $(table).DataTable({
            ...
            columns: [
                ...
                {
                    data: 'action', width: 'auto', title: "<?= !empty($this->lang->line('action')) ? $this->lang->line('action') : 'Action'; ?>",
...
<script>
    ...
    // geolocate.on('geolocate', (e) => { // trackuserlocationstart atau trackuserlocationend
    //     var coord = e.coords;
    //     console.log("location_start: ", `${coord.latitude}, ${coord.longitude} `, e.timestamp);
    ...
    function gmapStyleFix(type_style = 'street', e){
        var is_style_street_active = 'visible'; // default active street
        var is_style_satelit_active = 'none'; // default active street
    ...
    btn_home.removeAttribute("onclick"); // hapus dulu attribute onclick
    btn_home.addEventListener('click', saveChangesDraw);
    var cache_action = { 'home': null, 'lang': null };

    // fungsi untuk menampilkan popup untuk menyimpan polygon dulu
    function saveChangesDraw(e){
        e.preventDefault();
        ...
        if (uri2 == 'home' || uri2 == 'client' || uri2 == 'monitoring') {
            ...
            e.preventDefault();
            var getSel = draw.getSelected();
            console.log("selected: ", getSel);

            var getAll = draw.getAll();
            console.log("get all: ", getAll);
            cache_action.home = null; // reset

            if (getAll.features.length === 0) {
                window.location = '<?= base_url('home') ?>';
            } else {
                Swal.fire({
                    icon: 'info',
                    title: '<?= !empty($this->lang->line('notice_drawing')) ? $this->lang->line('notice_drawing') : 'Changes have not been saved. Are you sure you want to exit?'; ?>',
                    showCancelButton: true,
                    confirmButtonText: "<?= !empty($this->lang->line('notice_yes')) ? $this->lang->line('notice_yes') : 'Yes, Save'; ?>",
                    cancelButtonText: "<?= !empty($this->lang->line('notice_no')) ? $this->lang->line('notice_no') : "No, Don't Save"; ?>",
                    ...
                    if (r.isConfirmed) {
                        // tampilkan popup save file
                        $("#btn_save_polygon").click();
                        cache_action.home = '<?= base_url('home') ?>';
                    }
                    if (r.isDismissed) {
                        window.location = '<?= base_url('home') ?>';
                        cache_action.home = null; // reset
    ...
    // =====================================================
    var btn_change_lang = $(".language");
    var btn_en = $(btn_change_lang[0]);
    var btn_jp = $(btn_change_lang[1]);

    var saveEventLang = {
        "btn_en_onclick" : btn_en.attr("onclick"),
        "btn_jp_onclick" : btn_jp.attr("onclick"),
        "btn_en_href" : btn_en.attr("href"),
        "btn_jp_href" : btn_jp.attr("href"),
    }
    btn_change_lang.removeAttr("onclick"); // setelah disimpan baru hapus attribute nya
    btn_change_lang.on("click", saveChangesLang);

    console.log("btn_lang: ", btn_change_lang);
    console.log("saveEventLang: ", saveEventLang);

    // fungsi untuk menampilkan popup untuk menyimpan polygon dulu
    function saveChangesLang(e){
        e.preventDefault();
        var btnLang = $(e.currentTarget);
        var dataLang = btnLang.attr("data-lang");
        // console.log("saveChangesLang: ", e);
        ...
        var setHref = null;
        if (dataLang == 'eng') {
            setHref = saveEventLang.btn_en_href;
        } else if (dataLang == 'ja') {
            setHref = saveEventLang.btn_jp_href;
        }
        // btnLang.attr("onclick", setAction);
        // btnLang.attr("href", setHref);
        console.log("setHref: ", setHref);
        cache_action.lang = null; // reset
    ...
    map.on("draw.selectionchange", function (e){
        ...
        if (features.length != 0 && draw.getMode() != "direct_select") {
            ...
            // var convertDT = geoJSONToDataTable(features);
    ...
    $("#btn_save_polygon").on("click", function (e){
        ...
        Swal.fire({
            title: '<?= !empty($this->lang->line('enter_filename_to_save')) ? $this->lang->line('enter_filename_to_save') : 'Enter a file name to save the polygon'; ?>',
            input: 'text',
            showCancelButton: true,
            confirmButtonText: '<?= !empty($this->lang->line('submit')) ? $this->lang->line('submit') : 'Submit'; ?>',
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (!isAlertAlready) {
                    Swal.fire({
                        icon: 'success',
                        title: '<?= !empty($this->lang->line('polygon_saved')) ? $this->lang->line('polygon_saved') : 'Polygon saved successfully!'; ?>',
                        text: "<?= !empty($this->lang->line('do_you_want_to_create')) ? $this->lang->line('do_you_want_to_create') : 'Do you want to create a new drawing?'; ?>",
                        confirmButtonText: "<?= !empty($this->lang->line('yes_create_new')) ? $this->lang->line('yes_create_new') : 'Yes, I want to create a new'; ?>",
                        cancelButtonText: "<?= !empty($this->lang->line('no_see_result')) ? $this->lang->line('no_see_result') : 'No, I want to see the results'; ?>",
                        confirmButtonColor: "#20b2aa",
                        // cancelButtonColor: "#f46a6a",
                        showCancelButton: true,
                        allowOutsideClick: false,
                        ...
                        isAlertAlready = true;
                        if (e.isConfirmed) {
                            if (cache_action.home != null || cache_action.lang != null) {
                                if (cache_action.home != null) {
                                    // beralih ke home
                                    window.location = cache_action.home;
                                }
                                if (cache_action.lang != null) {
                                    // beralih ke change language
                                    window.location = cache_action.lang;
                                }
                            } else {
    ...
    function style_map(e){
        ...
        // nonaktifkan layer google 
        if (map.getSource(gmapLayerStreet)) {
            map.setLayoutProperty(gmapLayerStreet, 'visibility', 'none');
        }
        if (map.getLayer(gmapLayerSatelit)) {
            map.setLayoutProperty(gmapLayerSatelit, 'visibility', 'none');
        }
